...
test('Show me restores hidden upload controls without submitting a job',()=>{const {d,close}=setup();let submitted=0;d.querySelector('#go').addEventListener('click',()=>submitted++);d.querySelector('.hero').style.display='none';d.querySelector('[data-studio="guides"]').click();d.querySelector('[data-show="upload"]').click();assert.equal(d.querySelector('.hero').style.display,'');assert.equal(d.activeElement.id,'t-file');assert.equal(submitted,0);close()});
