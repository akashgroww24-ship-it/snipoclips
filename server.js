...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      scriptSrc: ["'self'", "'unsafe-inline'", "https://cdn.jsdelivr.net", "https://cdnjs.cloudflare.com", "https://connect.facebook.net", "https://www.googletagmanager.com"],
      scriptSrcAttr: ["'unsafe-inline'"], styleSrc: ["'self'", "'unsafe-inline'", "https://fonts.googleapis.com"],
      fontSrc: ["'self'", "https://fonts.gstatic.com"], imgSrc: ["'self'", "data:", "https://www.facebook.com", "https://*.google-analytics.com"],
      mediaSrc: ["'self'", "https://*.supabase.co", "blob:"], connectSrc: ["'self'", "https://*.supabase.co", "https://*.google-analytics.com", "https://connect.facebook.net"],
...
app.post('/api/billing/webhook', express.raw({ type: '*/*' }), async (req, res) => {
  ...
  try {
    ...
    const evt = JSON.parse(raw.toString('utf8')); const type = (evt.type || evt.event_type || '').toLowerCase(); const d = evt.data || evt;
    const meta = d.metadata || (d.subscription && d.subscription.metadata) || {}; const userId = meta.user_id, plan = meta.plan;
...
app.post('/admin/login', loginLimiter, body('email').isEmail(), body('password').isLength({ min: 1 }), (req, res) => {
  try {
    ...
    res.cookie('sc_admin', issueToken(req.body.email), COOKIE_OPTS); res.json({ ok: true });
  } catch (e) { console.error('[admin/login] error: ' + (e.message || e)); res.status(500).json({ error: 'Login error — check server logs' }); }
});
...
app.get('/admin/api/stats', requireAdmin, async (req, res) => {
  try {
    ...
    const [u, c, j] = await Promise.all([sbAdmin.from('profiles').select('*', head), sbAdmin.from('clips').select('*', head), sbAdmin.from('jobs').select('*', head)]);
    const { data: profs } = await sbAdmin.from('profiles').select('plan, created_at');
    const plans = { free: 0, single: 0, half: 0, full: 0 }; (profs || []).forEach(p => { plans[p.plan] = (plans[p.plan] || 0) + 1; });
    const days = []; for (let i = 13; i >= 0; i--) days.push(new Date(Date.now() - i * 86400000).toISOString().slice(0, 10));
    const sgn = Object.fromEntries(days.map(d => [d, 0])); (profs || []).forEach(p => { const d = (p.created_at || '').slice(0, 10); if (d in sgn) sgn[d]++; });
    const since14 = new Date(Date.now() - 14 * 86400000).toISOString(); const { data: clipRows } = await sbAdmin.from('clips').select('created_at').gte('created_at', since14);
    const cpd = Object.fromEntries(days.map(d => [d, 0])); (clipRows || []).forEach(x => { const d = (x.created_at || '').slice(0, 10); if (d in cpd) cpd[d]++; });
    let openReports = 0, reports = []; try { const r = await sbAdmin.from('reports').select('*', head).eq('status', 'open'); openReports = r.count || 0; const rr = await sbAdmin.from('reports').select('id,email,message,url,status,created_at').order('created_at', { ascending: false }).limit(25); reports = rr.data || []; } catch {}
...
app.get('/admin/api/config-status', requireAdmin, (req, res) => { const has = k => !!process.env[k]; res.json({ supabase: has('SUPABASE_URL') && has('SUPABASE_SERVICE_ROLE_KEY') && has('SUPABASE_ANON_KEY'), anthropic: has('ANTHROPIC_API_KEY'), groq: has('GROQ_API_KEY'), payments: has('DODO_API_KEY') && has('DODO_PRODUCT_SINGLE') && has('DODO_PRODUCT_HALF') && has('DODO_PRODUCT_FULL'), email: has('RESEND_API_KEY') || has('SMTP_HOST') }); });
