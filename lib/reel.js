...
function volume01(v, fallback){
  const n = Number(v);
  return Number.isFinite(n) ? Math.max(0, Math.min(1, n)) : fallback;
}

async function mixMusic(base, music, workDir, opts={}) {
  ...
  const voiceVol = volume01(opts.originalVolume, 1);
  const musicVol = volume01(opts.musicVolume, 0.13);
  await run(FFMPEG, ['-i',base,'-stream_loop','-1','-i',music,
    '-filter_complex',`[0:a]volume=${voiceVol.toFixed(3)}[voice];[1:a]volume=${musicVol.toFixed(3)},afade=t=in:st=0:d=0.8,afade=t=out:st=${fadeOut}:d=1.2[bed];[voice][bed]amix=inputs=2:duration=first:dropout_transition=2[a]`,
...
async function createReel(job, clips, opts={}) {
  ...
  try {
    ...
    if (music) out = await mixMusic(out,music,workDir,opts);
    ...
    const { error:dbErr } = await admin.from('clips').insert({
      ...
      edit:{ type:'reel', captionPreset:plan.captionPreset, transition:plan.transition, musicMood:plan.musicMood, sourceClipIds:ordered.map(c=>c.id), aiReason:plan.reason, ratio:'9:16', musicVolume:volume01(opts.musicVolume,0.13), originalVolume:volume01(opts.originalVolume,1) }
    });
