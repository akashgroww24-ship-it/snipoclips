...
// and have ffmpeg installed on the server.
//
...
const { downloadDirectVideo } = require('./remote-media');
...
// Strip anything sensitive before an error is ever stored on a job or shown to
// a user. In particular, signed direct-file URLs often carry bearer tokens in
// arbitrary query parameters.
function redact(s) {
  s = String(s == null ? '' : s);
  s = s.replace(/\/\/[^/\s@]+:[^/\s@]+@/g, '//');   // user:pass@ in any URL
  s = s.replace(/([?&][^=\s&]{1,80}=)[^&\s]+/g, '$1[redacted]');
...
async function fetchSource({ filePath, videoUrl }, workDir) {
  ...
  if (videoUrl) {
    try {
      return await downloadDirectVideo(videoUrl, workDir);
    } catch (e) {
      const clean = new Error(e.message || 'Could not import that direct video file.');
      clean.code = e.code || 'download_failed';
      clean.detail = redact(e.message || '');
      throw clean;
    }
  }
...
module.exports = { processJob, runTestJob, renderClip, renderMaster, renderEdit, buildASS, probeDuration, tightenClip, computeKeep, buildFaceCrop, selectHighlights, extractAudio, transcribe, transcribeLong, planAudioChunks, detectScript, transcriptConfidence, buildCandidateWindows, heuristicPick, activeJobs: () => ACTIVE, maxConcurrency: MAX_CONCURRENCY };
