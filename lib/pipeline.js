...
// 1) Get the source video into a local file.
...
async function renderClip(input, words, clip, idx, workDir, opts = {}) {
  ...
  if (opts.brollFile && opts.brollStart != null && opts.brollEnd != null && opts.brollEnd > opts.brollStart) {
    ...
    const fc = `[0:v]scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H}[base];`
             + `[1:v]scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H},setpts=PTS-STARTPTS+${bs}/TB[bv];`
    ...
    a.push('-c:v','libx264','-preset','veryfast','-crf','23','-c:a','aac','-b:a','128k','-movflags','+faststart','-y', out);
    try { await run(FFMPEG, a, { cwd: workDir }); return out; }
    catch (e) { console.warn('[broll] composite failed, plain render: ' + (e.message || e)); }
  }
  const reframe = opts.reframe || `scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H}`;
  let vf = `${reframe},${capChain}`;
  const args = ['-ss', String(clip.start), '-t', dur, '-i', input, '-vf', vf];
  if (opts.enhance) args.push('-af', AUDIO_ENHANCE);
  args.push('-c:v', 'libx264', '-preset', 'veryfast', '-crf', '23', '-c:a', 'aac', '-b:a', '128k', '-movflags', '+faststart', '-y', out);
...
async function renderMaster(input, m0, m1, idx, workDir, opts = {}) {
  ...
  const [W,H] = dimsOf(opts.ratio);
  const args = ['-ss', String(m0), '-t', dur, '-i', input, '-vf', (opts.reframe || `scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H}`)];
  if (opts.enhance) args.push('-af', AUDIO_ENHANCE);
  args.push('-c:v','libx264','-preset','veryfast','-crf','23','-c:a','aac','-b:a','128k','-movflags','+faststart','-y', out);
  await run(FFMPEG, args, { cwd: workDir });
  return out;
...
async function tightenClip(input, clipStart, clipEnd, words, workDir, idx, enhance){
  ...
  await run(FFMPEG, ['-ss', String(clipStart), '-t', String(clipEnd-clipStart), '-i', input,
    '-vf', vf, '-af', af, '-c:v','libx264','-preset','veryfast','-crf','20','-c:a','aac','-b:a','128k','-y', out], { cwd: workDir });
...
async function buildFaceCrop(input, ss, dur, W, H){
  try {
    ...
    return `crop=${cw}:${ch}:x='${xexpr}':y=${yc},scale=${W}:${H}`;
  } catch (e) { return null; }
}
