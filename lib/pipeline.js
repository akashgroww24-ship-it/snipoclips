...
// ============================================================
//  CLIP STYLE PRESETS  (Clean / Punchy / Sigma / Meme)
//  Pure ffmpeg filters — no new dependency.
//  Applied AFTER reframe, BEFORE captions.
// ============================================================
const CLIP_STYLES = {
  clean: {
    grade: '',
  ...
  sigma: {
    // crushed blacks + desaturated + cool tint + vignette
    grade: 'eq=contrast=1.35:brightness=-0.04:saturation=0.62,'
         + 'colorbalance=rs=-0.05:bs=0.08,'
         + 'vignette=PI/5',
    punch: true
  },
  meme: {
    grade: 'eq=contrast=1.22:saturation=1.35',
    punch: true
  }
};

// Small zoom kick on the loudest moments — what makes an edit feel "edited".
// peaks: seconds relative to clip start. w/h: output dims.
function punchFilter(peaks, dur, w, h) {
  if (!Array.isArray(peaks) || !peaks.length) return '';
  const terms = peaks
    .filter(t => t > 0.2 && t < dur - 0.2)
    .slice(0, 24)
    .map(t => `if(between(t,${t.toFixed(2)},${(t + 0.25).toFixed(2)}),`
            + `1+0.06*sin((t-${t.toFixed(2)})/0.25*PI),0)`)
    .join('+');
  if (!terms) return '';
  return `,scale=iw*1.02:ih*1.02,crop=iw/1.02:ih/1.02,`
       + `zoompan=z='1+(${terms})':d=1:s=${w}x${h}:fps=30`;
}

// Pick local loudness peaks inside a clip window from the ebur128 timeline.
// energy: [{t, e}] from analyzeAudioEnergy (e = LUFS, higher = louder)
function peaksForClip(energy, start, end) {
  const win = (energy || []).filter(x => x.t >= start && x.t <= end);
  const out = [];
  for (let i = 1; i < win.length - 1; i++) {
    const p = win[i - 1], c = win[i], n = win[i + 1];
    if (c.e > p.e && c.e > n.e && c.e > -22) {
      const rel = +(c.t - start).toFixed(2);
      if (!out.length || rel - out[out.length - 1] > 0.6) out.push(rel);
    }
  }
  return out;
}
...
// Preserve music/ambience while evening out the source soundtrack. This does
// not separate speech from music; the speech preset above targets steady noise.
const AUDIO_BACKGROUND = 'highpass=f=45,acompressor=threshold=-20dB:ratio=2:attack=25:release=300,loudnorm=I=-16:TP=-1.5:LRA=11';
const audioFilter = mode => mode === 'background' ? AUDIO_BACKGROUND : AUDIO_ENHANCE;
...
// captioned vertical video). 'fast' reduces the wait for multi-clip jobs while
// retaining CRF 18 quality; hosts with more CPU can choose 'medium' via env.
...
const X264_PRESET = process.env.X264_PRESET || 'fast';
...
async function renderClip(input, words, clip, idx, workDir, opts = {}) {
  ...
  if (opts.brollFile && opts.brollStart != null && opts.brollEnd != null && opts.brollEnd > opts.brollStart) {
    ...
    const fc = `[0:v]scale=${W}:${H}:force_original_aspect_ratio=increase:flags=lanczos,crop=${W}:${H}[base];`
             ...
             + `[ov]${(CLIP_STYLES[opts.clipStyle] || CLIP_STYLES.clean).grade ? (CLIP_STYLES[opts.clipStyle].grade + ',') : ''}${capChain}[outv]`;
    const a = ['-ss', String(clip.start), '-t', dur, '-i', input, '-i', opts.brollFile, '-filter_complex', fc, '-map', '[outv]', '-map', '0:a'];
    if (opts.enhance) a.push('-af', audioFilter(opts.audioMode));
  ...
  // ---- clip style (grade + optional beat punch) ----
  const style = CLIP_STYLES[opts.clipStyle] || CLIP_STYLES.clean;
  const grade = style.grade ? `,${style.grade}` : '';
  const punch = style.punch ? punchFilter(opts.clipPeaks, Number(dur), W, H) : '';

  let vf = `${reframe}${grade}${punch},${capChain}`;
  const args = ['-ss', String(clip.start), '-t', dur, '-i', input, '-vf', vf];
  if (opts.enhance) args.push('-af', audioFilter(opts.audioMode));
...
async function tightenClip(input, clipStart, clipEnd, words, workDir, idx, enhance, audioMode){
  ...
  const af = `aselect='${sel}',asetpts=N/SR/TB` + (enhance ? ',' + audioFilter(audioMode) : '');
...
async function processJob(job, source) {
  ...
  ACTIVE++;
  // These counters are read by catch, so they must live in the function scope
  // rather than inside the try block.
  ...
  try {
    ...
    let highlights = [];
    let energyTimeline = [];   // filled by the fallback, or lazily for punch styles
    ...
    if (!highlights.length && vdur) {
      ...
      energyTimeline = energy;   // reused below for beat-punch styling
    ...
    // Beat-punch styles need the loudness timeline. Compute once if missing.
    const wantsPunch = !!(CLIP_STYLES[source.clipStyle] || {}).punch;
    if (wantsPunch && !energyTimeline.length) {
      try { energyTimeline = await analyzeAudioEnergy(input); }
      catch { energyTimeline = []; }
    }
    ...
    for (let i = 0; i < highlights.length; i++) {
      ...
      let tight = null;
      if (source.fillers) {
        try { tight = await tightenClip(input, c.start, c.end, transcript.words, workDir, i, source.enhance, source.audioMode); }
      ...
      const file = await renderClip(baseFile, baseWords, clipObj, i, workDir, { plan, captionStyle: source.captionStyle, enhance: baseEnhance, audioMode: source.audioMode, brollFile, brollStart, brollEnd, ratio: source.ratio, hook: source.hook ? c.title : '', highlight: source.highlight, progress: source.progress, emoji: source.emoji, karaoke: source.karaoke, reframe: brollFile ? null : reframe,
        clipStyle: source.clipStyle,
        clipPeaks: wantsPunch ? peaksForClip(energyTimeline, clipObj.start, clipObj.end) : [] });
      ...
      try {
        if (regionMaster) {
          const mfile = await renderMaster(baseFile, baseStart, baseEnd, i, workDir, { enhance: baseEnhance, audioMode: source.audioMode, ratio: source.ratio, reframe: brollFile ? null : reframe });
          ...
          const mfile = await renderMaster(input, m0, m1, i, workDir, { enhance: source.enhance, audioMode: source.audioMode, ratio: source.ratio });
          master_path = await uploadMaster(mfile, job.user_id, job.id, i);
      ...
      const { data: _ins, error: _insErr } = await admin.from('clips').insert({
      ...
      // Fail loudly. A silent insert failure previously produced jobs marked
      // "done" with clips in storage but zero rows in the database.
      if (_insErr) {
        console.error('[clips.insert] job=' + job.id + ' clip=' + i + ' failed: ' + (_insErr.message || _insErr));
        throw new Error('Could not save clip ' + (i + 1) + ': ' + (_insErr.message || 'database insert failed'));
      }
      ...
      made++;
      await setStatus(job.id, { stage:'rendering', clips_count: made });
    }
    ...
    // Give the minutes back when the job produced nothing. Charging for a
    // video that yielded zero clips is what generates refund requests.
    if (chargedMinutes > 0 && made === 0) {
      try {
        const { data: p } = await admin.from('profiles')
          .select('minutes_used').eq('id', job.user_id).single();
        if (p) {
          const back = Math.max(0, Number(p.minutes_used || 0) - chargedMinutes);
          await admin.from('profiles').update({ minutes_used: back }).eq('id', job.user_id);
          console.log(`[quota] job=${job.id} refunded ${chargedMinutes.toFixed(2)} min (0 clips produced)`);
        }
      } catch (re) {
        console.error('[quota] refund failed for job=' + job.id + ': ' + (re.message || re));
      }
    }
...
async function runTestJob(source, outDir) {
  ...
  try {
    ...
    for (let i = 0; i < highlights.length; i++) {
      const c = highlights[i];
      const file = await renderClip(input, transcript.words, c, i, workDir, { plan: (typeof plan!=='undefined'?plan:'free'), captionStyle: source.captionStyle, enhance: source.enhance, clipStyle: source.clipStyle });
